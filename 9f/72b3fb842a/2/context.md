...
- CLAUDE.md (secções Component Patterns, Token System, Mosaic Icon System)
- src/styles/tokens.css (tokens disponíveis)
- src/components/mosaic-icon.astro (API atual após Session 2)
- src/lib/mosaic-icons.ts (confirma que bagIcon e arrow existem)
- src/components/highlight-card.astro (padrão de componente existente)
- src/pages/index.astro (secção do hero — linhas onde .highlight-row e --mosaic-state estão definidos)
